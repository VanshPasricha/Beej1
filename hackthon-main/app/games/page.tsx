...
import { Home, RefreshCcw, Puzzle, Calculator, Sprout } from "lucide-react"
      ...
      <DashboardLayout>
        <div className="space-y-6 p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="outline" onClick={() => setSession((s) => s + 1)} aria-label="Restart" title="Restart">
          ...
          <div>
            ...
            {selectedGame === "eco-farm-simulator" && <EcoFarmSimulator key={session} />}
          </div>
        </div>
      </DashboardLayout>
    )
          ...
          {miniGames.map((game) => {
            ...
            return (
              <Card
                ...
                onClick={onActivate}
                className={`group h-full border-2 rounded-xl transition shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 ${game.borderColor}`}
              >
                <div className="flex h-full flex-col">
                  <CardHeader className="p-5">
                    <div className="flex items-start gap-4">
                      <div className={`p-3 rounded-full ${game.bgColor} ring-1 ring-black/5 shrink-0`}>
                        <Icon className={`w-8 h-8 ${game.color}`} />
                      </div>
                      <div className="min-w-0">
                        <CardTitle className="text-xl leading-6 line-clamp-2">{game.title}</CardTitle>
                  ...
                  <CardContent className="mt-auto px-5 pb-5">
                    <div className="flex items-center justify-between text-xs text-gray-600">
                      <span className="inline-flex items-center gap-2">
                        <Badge variant="outline" className="mr-1">{game.difficulty}</Badge>
                        <span className="text-gray-500">{game.duration}</span>
                      </span>
                      <span className="font-medium text-gray-700">{game.points} pts</span>
                    </div>
                  </CardContent>
                </div>
              </Card>
...
function MatchPairsGame() {
  ...
  const onFlip = (id: number) => {
    ...
    if (flipped.length === 1) {
      ...
      if (a && b && Math.floor(a.id / 2) === Math.floor(b.id / 2)) setMatched((m) => [...m, firstId, id])
      setFlipped([])
    } else setFlipped([id])
  }
...
function SumPuzzleGame() {
  ...
  const submit = () => setMessage(currentSum === target ? "success" : "try")
  const reset = () => {
    setSelected([])
    setMessage(null)
  }
